...
func ListLinhas(w http.ResponseWriter, r *http.Request) {
	setupResponse(&w, r)
	enableCors(&w)
	var filtro dbfunctions.ProdutoFiltro
	err := json.NewDecoder(r.Body).Decode(&filtro)
	...
	faturas, err := dbfunctions.GetFatura(filtro)
	if err != nil {
		log.Fatalf("Error getting fatura: %v", err)
	}

	log.Println("Fetching all fatura")
	artigos, err := dbfunctions.GetLinhasFatura(faturas[0].Id)
	if err != nil {
		http.Error(w, "Error fetching fatura", http.StatusInternalServerError)
